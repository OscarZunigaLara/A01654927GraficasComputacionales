...
function main() 
{
    ...
    ///GENERA EL OBJETO DEL ROMBO 
    let rhombus = createRhombus(gl);
    //GENERA EL OBJETO DEL PACMAN
    let circlePacman = createSphere(gl, 1.5);

    mat4.identity(modelViewMatrix);
    
    mat4.translate(modelViewMatrix, modelViewMatrix, [-1.1 , .7, -3.333]);
    draw(gl, square);
    ...
    ///SE MUEVE DEBAJO DEL CUADRADO "square"
    mat4.translate(modelViewMatrix, modelViewMatrix, [-3.3,-1.6, -3.333]);
    //DIBUJA EL ROMBO 
    draw(gl, rhombus);

    ////SE MUEVE DEBAJO DEL TRIANGULO "triangle"
    mat4.translate(modelViewMatrix, modelViewMatrix, [5.5,-.5, -3.333]);
    ///dibuja el objeto "circlePacman"
    draw(gl, circlePacman);
...
///FUNCION PARA TRANSFORMAR GRADOS A RADIANES, recibe  degrees y regresa radianes
function degrees_to_radians(degrees){
    let degsToRads = (degrees * Math.PI) / 180.0;
    return degsToRads
}
